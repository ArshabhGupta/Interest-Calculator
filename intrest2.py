...
def calculate_interest():
    try:
        ...
        amount2 = compound_interest + principal

        # Display results
        si_label.config(text=f"Simple Interest: {simple_interest:.2f}")
        ci_label.config(text=f"Compound Interest: {compound_interest:.2f}")
        #amount_label.config(text=f"Simple Interest Amount: {amount:.2f})
       #amount2_label.config(text=f"Compound Interest: {amount2:.2f})
...
amount_label = tk.Label(window, text = "Amount for SI: ")
amount_label.grid(row=5, column=0, columnspan=2, padx=10, pady=10)

#ci_label = tk.Label(window, text="Compound Interest: ")
#ci_label.grid(row=6, column=0, columnspan=2, padx=10, pady=10)

amount_label2 = tk.Label(window, text = "Amount for CI: ")
amount_label2.grid(row=7, column=0, columnspan=2, padx=10, pady=10)
